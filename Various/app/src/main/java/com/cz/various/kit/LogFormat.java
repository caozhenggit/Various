...
public class LogFormat {
    ...
    public static String formatBorder(String[] segments) {
        ...
        msgBuilder.append(TOP_HORIZONTAL_BORDER).append(FPrinter.lineSeparator);
        for (int i = 0; i < nonNullCount; i++) {
            msgBuilder.append(appendVerticalBorder(nonNullSegments[i]));
            if (i != nonNullCount - 1) {
                msgBuilder.append(FPrinter.lineSeparator).append(DIVIDER_HORIZONTAL_BORDER)
                        .append(FPrinter.lineSeparator);
            } else {
                msgBuilder.append(FPrinter.lineSeparator).append(BOTTOM_HORIZONTAL_BORDER);
            }
        }
    ...
    private static String appendVerticalBorder(String msg) {
        ...
        String[] lines = msg.split(FPrinter.lineSeparator);
        for (int i = 0, N = lines.length; i < N; i++) {
            if (i != 0) {
                borderedMsgBuilder.append(FPrinter.lineSeparator);
            }
            String line = lines[i];
